...
namespace iLgs.Services.Interfaces
{
    public class PoIssuanceService : IPoIssuanceService
    {
        ...
        //{
        //    var IsAdmin = await userService.IsAdmin(userId);
        //    var data = _db.OrderItems.AsNoTracking()
        ...
        //public async ValueTask<PoIssuanceVM> GetByOrderItemIdAsync(Guid? orderItemId)
        //{
        //    var data = await _db.OrderItems.Where(w => w.Id == orderItemId).AsNoTracking()
        //            .Select(s => new PoIssuanceVM
        //            {
        //                Id = s.Id,
        //                RisItemId = s.RequestItem.RisItemId,
        ...
        _vmExceptionService.TryCatchAsync(async () =>
         {
             ...
             var orderItem = await GetOrderItemByAirItemIdAsync(model.Id);
             ...
             {                 
                 ...
                 model.PropNo = NextPropNo(orderItem.AirDate.Value.Year.ToString(), orderItem.StockNo, model.LocationCode);
                 var entity = new RisIssued()
                 {
                     Id = Guid.NewGuid(),
                     RisItemId = orderItem.RisItemId,
