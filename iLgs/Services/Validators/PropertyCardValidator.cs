...
{
    public class PropertyCardValidator : AbstractValidator<PropertyCardVM>
    {
        ...
        public PropertyCardValidator(AppManEntities db)
        {
            ...
            RuleFor(m => m.PsNo)
                .NotEmpty().WithMessage("Property Number is required.");
            ...
            RuleSet("Create", () =>
            {
               ...
               .MustAsync(async (psNo, cancellation) =>
                ...
                .WithMessage(m => $"Property Number '{m.PsNo}' already exists.");
            });
