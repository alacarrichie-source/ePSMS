...
{
    public interface IPsCardItemExtnLandValidator
    {
        void ValidateOnCreate(PsCardItemExtnLandVM model);
        void ValidateOnUpdate(PsCardItemExtnLandVM model);
        void ValidateOnDelete(PsCardItemExtnLandVM model);
    }

    public class PsCardItemExtnLandValidator : BaseValidator, IPsCardItemExtnLandValidator
    {
        private readonly AppManEntities _db;
        private readonly GetDisplayNameDelegate _getDisplayName;

        public PsCardItemExtnLandValidator(AppManEntities db)
        {
            _db = db;
            _getDisplayName = propertyName => Utility.GetDisplayName<PsCardItemExtnLandVM>(propertyName);
        }

        public void ValidateOnCreate(PsCardItemExtnLandVM model)
        {
        ...
        public void ValidateOnUpdate(PsCardItemExtnLandVM model)
        {
            ValidateIfNull(model);
        ...
        public void ValidateOnDelete(PsCardItemExtnLandVM model)
        {
            ValidateIfNull(model);
            ValidateRecord((Guid)model.Id);
            ValidateIfPosted(model);
        ...
        public void ValidateFieldsOnCreateUpdate(PsCardItemExtnLandVM model, Mode mode)
        {
            var ex = new InvalidModelException();

            if (string.IsNullOrWhiteSpace(model.PIN))
            {
                var data = _db.PsCardItemExtns.OfType<PsCardItemExtnLand>().FirstOrDefault(f => f.PIN == model.PIN);
                if (data != null && (mode == Mode.ADD || data.Id != model.Id))
                {
                    ex.UpsertDataList(_getDisplayName(nameof(model.PIN)), "Already Exists.");
                }
            }
        ...
        private static void ValidateIfNull(PsCardItemExtnLandVM model)
        {
            if (model is null)
            {
        ...
        private void ValidateIfPosted(PsCardItemExtnLandVM model)
        {
