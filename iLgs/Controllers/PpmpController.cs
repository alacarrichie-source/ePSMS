...
namespace iLgs.Controllers
{
    ...
    public class PpmpController : BaseController
    {
        ...
        public ActionResult _ItemUsage(Guid? ppmpItemId)
        {
            ViewData["ppmpItemId"] = ppmpItemId;
            return PartialView();
        }

        public ActionResult _ItemUsageRead([DataSourceRequest] DataSourceRequest request, Guid? ppmpItemId)
        {
            var data = _ppmpService.PPMPItem.PPMPItemUsage.GetAll(ppmpItemId);

            return new JsonNetResult { Data = data.ToDataSourceResult(request), JsonRequestBehavior = JsonRequestBehavior.AllowGet, Settings = { ReferenceLoopHandling = ReferenceLoopHandling.Ignore } };
        }

        [AcceptVerbs(HttpVerbs.Post)]
        public async Task<ActionResult> _ItemUsageCreate([DataSourceRequest] DataSourceRequest request, PPMPItemUsageVM model)
        {
            try
            {
                var access = await Access(User.Identity.GetUserId(), "ppmp");
                ...
                if (model != null && ModelState.IsValid)
                {
                    ...
                    model = await _ppmpService.PPMPItem.PPMPItemUsage.CreateAsync(model, user, date);
                }
            }
            catch (ValidationException validationException) when (validationException.InnerException is InvalidModelException)
            {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public async Task<ActionResult> _ItemUsageUpdate([DataSourceRequest] DataSourceRequest request, PPMPItemUsageVM model)
        {
            try
            {
                Task<Access> accessTask = Access(User.Identity.GetUserId(), "ppmp");
                ...
                if (ModelState.IsValid)
                {
                    ...
                    model = await _ppmpService.PPMPItem.PPMPItemUsage.UpdateAsync(model, user, date);
                }
            }
            catch (ValidationException validationException) when (validationException.InnerException is InvalidModelException)
            {
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public async Task<ActionResult> _ItemUsageDestroy([DataSourceRequest]DataSourceRequest request, PPMPItemUsageVM model)
        {
            try
            {
                Task<Access> accessTask = Access(User.Identity.GetUserId(), "ppmp");
                ...
                else
                {
                    ...
                    model = await _ppmpService.PPMPItem.PPMPItemUsage.DeleteAsync(model, user, date);
                    // TO DO: update stocks
                }
            }
            catch (ValidationException validationException)
