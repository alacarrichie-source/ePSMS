...
namespace iLgs.Controllers
{
    public class SysCodesController : Controller
    {
        ...
        public ActionResult GetSysCodes(string text)
        {
            ...
            IEnumerable<SysCode> model = Enumerable.Empty<SysCode>().AsQueryable();
            HttpResponseMessage responseMessage = client.GetAsync("syscodes_/" + userId).Result;
            if (responseMessage.IsSuccessStatusCode)
            {
                var responseData = responseMessage.Content.ReadAsStringAsync().Result;
                model = JsonConvert.DeserializeObject<List<SysCode>>(responseData);
                if (!string.IsNullOrEmpty(text))
                {
                    model = model.Where(w => w.SysCode1.Contains(text));
                }
            }

            var retVal = model.Select(c => new { Code = c.SysCode1, Description = c.SysDescription }).ToList();
